...
function navBar(type='public'){
  ...
  return `<div class="nav"><div class="container nav-inner">
    ...
    <div class="nav-actions">
      ${logged ? `<button class="btn btn-outline optional" onclick="go(state.role || 'farmer')">Dashboard</button><button class="btn btn-primary btn-sm" onclick="logout()">Log out</button>` : `<button class="btn btn-outline optional" onclick="go('login')">Log in</button><button class="btn btn-primary" onclick="go('register')">Register</button>`}
    </div>
  </div></div>`;
}
...
function pageShell(role, active, body){
  const menus = role==='farmer' ? [
    ...
    ['dashboard','⌂','Dashboard'],['requirements','📋','Requirements'],['createReq','＋','Post Requirement'],['farmerLots','🌾','Farmer Lots'],['offers','🤝','Offers'],['transactions','✓','Transactions'],['profile','👤','Profile']
...
function login(){
  ...
  return `${navBar()}<div class="auth-wrap"><div class="card auth-card"><h1>Welcome to Farmora</h1><p class="sub">Log in to your account.</p><form class="form" onsubmit="event.preventDefault();loginGo()"><div class="field"><label>Mobile or email</label><input id="loginId" placeholder="${role==='farmer'?'farmer@farmora.demo':role==='buyer'?'buyer@farmora.demo':'admin@farmora.demo'}" required></div><div class="field"><label>Password</label><input id="loginPwd" type="password" placeholder="demo123" required></div><button class="btn btn-primary" type="submit">Log in to dashboard →</button></form></div></div>`;
}
function register(){return `${navBar()}<div class="auth-wrap"><div class="card auth-card"><h1>Create your Farmora account</h1><p class="sub">Use this demo form to explore the farmer or buyer experience.</p><form class="form" onsubmit="event.preventDefault();registerGo()"><div class="field"><label>Full name</label><input id="regName" placeholder="e.g. Ramesh Kumar" required></div><div class="field"><label>Phone</label><input id="regPhone" placeholder="10-digit mobile" required></div><div class="field"><label>Role</label><select id="regRole"><option value="FARMER">Farmer / FPO</option><option value="BUYER">Buyer</option><option value="ADMIN">Manager (Admin)</option></select></div><div class="field"><label>Location</label><input id="regLoc" placeholder="Hyderabad, Telangana" required></div><div class="field"><label>Password</label><input id="regPwd" type="password" required></div><button class="btn btn-primary" type="submit">Create demo account</button></form></div></div>`;}
...
async function registerGo() {
  ...
  if (res.success) {
    ...
    toast('Register failed: ' + (res.error.message.length ? JSON.stringify(res.error.message) : res.error.message));
  }
}
function logout(){
...
function buyerBody(){const t=state.buyerTab;if(t==='requirements')return buyerRequirements();if(t==='createReq')return buyerCreateReq();if(t==='farmerLots')return buyerFarmerLots();if(t==='offers')return buyerOffers();if(t==='transactions')return buyerTransactions();if(t==='profile')return profilePage('buyer');return buyerDashboard()}
...
async function loadProfile() {
  ...
  if (res.success) {
    ...
    state.profileError = res.error?.message || 'Failed to load profile';
  }
  render();
}
...
function profilePage(role) {
  ...
  const photo = state.profilePhotoPreview || p.profilePhotoUrl;
  const avatarHtml = photo ? `<img src="${photo}" alt="Profile">` : `<span>${esc((p.name||'?').charAt(0).toUpperCase())}</span>`;
